...
export const dfService = () => {
  ...
  const getCharacterCreatureInfo = async (characterId, serverId) => {
    const path =
      import.meta.env.VITE_API_URL + `creature/${characterId}/${serverId}`;
    console.log(path);
    try {
      const res = await axios.get(path);
  ...
  const getCharacterAvatarInfo = async (characterId, serverId) => {
    const path =
      import.meta.env.VITE_API_URL + `avatar/${characterId}/${serverId}`;
    try {
      const res = await axios.get(path);
  ...
  const getCharacterSwitchingInfo = async (characterId, serverId) => {
    const path =
      import.meta.env.VITE_API_URL + `switching/${characterId}/${serverId}`;
    try {
      const res = await axios.get(path);
  ...
  const getCharacterItemInfo = async (serverId, characterId) => {
    const path =
      import.meta.env.VITE_API_URL + `charcaterItem/${serverId}/${characterId}`;
    try {
      const res = await axios.get(path);
      return res.data;
    } catch (error) {
